...
2. 下载专用升级包的两个分卷，放在同一目录后右键 `.001` 全部解压，不要直接在压缩包里运行。
3. 双击升级包根目录的 `OliviaLetterUpdater.exe`，等待提示升级完成。
...
如果旧版没有安装器，也可以直接运行新文件夹根目录的 `OliviaLetterOverlay.exe`；但想继续使用游戏伴随启动，建议按上面的 `Install.cmd` 流程升级。
...
普通回信和主动来信不主动发送输出 token 上限，具体长度由所选模型或服务决定；正文不会再被应用层按固定字数裁剪。DeepSeek 通道显式关闭默认思考；智谱保留低推理强度，不向要求思考的模型发送禁用参数。接口仍可能受模型上下文窗口或中转服务限制；检测到长度截断、未完成响应、仅思考无正文或返回格式异常时，会显示具体原因，不会把半封回信当作完整回信保存，也不会自动重试产生额外调用。
...
回信设置还可以控制历史上下文：`-1` 带入不限天数，`0` 不带历史，正整数只带入最近几天。后台自动整理默认每 20 封用户来信执行一次，把较早往来压缩成摘要并提炼重要事实；填 `0` 可关闭自动整理。摘要和记忆按角色独立保存。
